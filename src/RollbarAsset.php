...
{
    /**
     * @var null|array|callable Extra payload to add
     */
    public $payload;
    /**
     * @var array rollbar js configuration
    ...
    /**
     * @var string Js rollbar config template
     */
    private $jsConfig = '
    ...
    public function init()
    {
        if (empty($this->config['accessToken'])) {
            throw new \Exception('Rollbar js requires accessToken!');
        }
    ...
    {
        return ArrayHelper::getValue($this->config, 'payload.environment');
    }

    public function getAccessToken()
    {
        return ArrayHelper::getValue($this->config, 'accessToken');
    }

    public function buildJs()
    {
        $config     = $this->config;
        $payload    = ArrayHelper::getValue($config, 'payload', []);
        $addPayload = $this->buildPayload();
        if (! empty($addPayload)) {
            $config['payload'] = ArrayHelper::merge((array) $payload, (array) $addPayload);
        }
        $js = strtr($this->jsConfig, [
    ...
    private function buildPayload()
    {
        ...
        if (! empty($this->payload)) {
            if (is_callable($this->payload)) {
                $payload = ArrayHelper::merge($payload, (array) call_user_func($this->payload, $this));
            } elseif (is_array($this->payload)) {
                $payload = ArrayHelper::merge($payload, $this->payload);
            }
        }
        return $payload;
    }
